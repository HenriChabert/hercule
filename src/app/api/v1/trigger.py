...
@router.get("/triggers")
async def get_triggers(db: Annotated[AsyncSession, Depends(async_get_db)], event: EventType | None = None):
    trigger_ctrl = TriggerController(db)
...
async def run_trigger(trigger_id: str, context: Annotated[Mapping[str, Any], Body()], db: Annotated[AsyncSession, Depends(async_get_db)]):
    trigger_ctrl = TriggerController(db)
    return await trigger_ctrl.trigger(trigger_id, context)

@router.post("/triggers/event")
async def trigger_event(event: Annotated[EventType, Body()], context: Annotated[EventContext, Body()], db: Annotated[AsyncSession, Depends(async_get_db)]):
    trigger_ctrl = TriggerController(db)
