...
async def send_webpush(subscription_data: dict[str, Any], payload: Mapping[str, Any]) -> bool:
  try:
    subscription = WebPushSubscription.model_validate(subscription_data)
    message = wp.get(message=cast(dict[str, Any], payload), subscription=subscription)
