...
def name_field_factory(**kwargs: Any):
    return Field(
        description="The name of the webhook", examples=["My Webhook"], **kwargs
    )
...
def auth_token_field_factory(**kwargs: Any):
    return Field(
        description="The auth token of the webhook", examples=["sijwdnu3"], **kwargs
    )
...
class WebhookBase(BaseModel):
    ...
    name: str = name_field_factory(
        default_factory=lambda: f"Webhook created at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
